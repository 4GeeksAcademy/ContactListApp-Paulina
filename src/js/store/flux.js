const getState = ({ getStore, getActions, setStore }) => {
	return {
		...
		actions: {
			...
			createSlug: () => {
					fetch('https://playground.4geeks.com/contact/agendas/PaulinaPereyra', {
						method: "POST",
						headers: {
			...
			getContacts: () => {
					fetch('https://playground.4geeks.com/contact/agendas/PaulinaPereyra/contacts')
					.then((response) => {
			...
			deleteContact: (contactId) => {
				fetch(`https://playground.4geeks.com/contact/agendas/PaulinaPereyra/contacts/${contactId}`, {
					method: "DELETE"	
				})
				.then((response) => { 
					const store = getStore();
